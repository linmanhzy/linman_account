...
**防线 1**：`network_security_config.xml` — Android 级别明文放行
- 在 `res/xml/` 下生成 `network_security_config.xml`，声明 `cleartextTrafficPermitted="true"`
- 注入 `AndroidManifest.xml` 的 `<application>` 标签：`android:networkSecurityConfig="@xml/network_security_config"`

**防线 2**：`RustWebView.kt` — WebView 引擎级别混合内容放行（2026-07-23 新增）
- Tauri v2 生成的 `RustWebView.kt` 未设置 `mixedContentMode`，WebView 默认 `MIXED_CONTENT_NEVER_ALLOW`
- 注入脚本在 `init` 块末尾插入 `settings.mixedContentMode = WebSettings.MIXED_CONTENT_ALWAYS_ALLOW`
- **这是最关键的修复**：防线 1 不一定能覆盖所有 Android 版本/WebView 实现的混合内容策略

**防线 3**：`build.gradle.kts` — App 级别明文放行（原有）
- `manifestPlaceholders["usesCleartextTraffic"] = "true"` 写入 release 块与 defaultConfig 块
...
| `_inject_nsc_into_manifest()` | 在 `AndroidManifest.xml` 注入 `android:networkSecurityConfig` |
| `_inject_webview_mixed_content()` | **NEW** 在 `RustWebView.kt` 注入 `mixedContentMode = ALWAYS_ALLOW` |
| `inject_all()` | 统一入口：gradle + NSC + WebView 三道注入 |

> 注意：所有注入具有**字节级幂等性**（13 个单测覆盖），重复调用不会产生重复行。
