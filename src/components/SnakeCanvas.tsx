...
/** 暴露给父组件（移动端触屏）的命令式控制接口 */
export interface SnakeCanvasHandle {
  /** 转向（带防 180° 掉头），参数为滑动方向 */
  turn: (dir: SwipeDirection) => void
  /** 开始 / 暂停 / 继续（等同于桌面空格键） */
  action: () => void
}
...
const SnakeCanvas = forwardRef<SnakeCanvasHandle, Props>(
  ({ onScoreChange, onGameStateChange }, ref) => {
    ...
    // ====== 转向（防 180° 掉头）======
    const turn = useCallback((dir: SwipeDirection) => {
      if (gameStateRef.current !== 'running') return
      const map: Record<SwipeDirection, Point> = {
        up: { x: 0, y: -1 },
        down: { x: 0, y: 1 },
        left: { x: -1, y: 0 },
        right: { x: 1, y: 0 },
    ...
    // ====== 生命周期 ======
    useImperativeHandle(ref, () => ({ turn, action }), [turn, action])
